#!/usr/bin/env bash
# scripts/build-swift.sh — Build the Speakey Swift app
set -euo pipefail
...
if [ -f "$BINARY" ]; then
    ...
    echo ""
echo "Source-first install — run with:"
echo "  $BINARY"
echo ""
echo "First run downloads the CoreML model (~2.5GB)."
echo "After granting permissions, quit and reopen the app."
echo ""
...
echo ""
echo "Tip: point an AI agent at this repo and ask it to run ./scripts/build-swift.sh"
else
